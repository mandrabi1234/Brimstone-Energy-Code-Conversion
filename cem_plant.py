...
#--------------------------------------------------------------------------------------------
    ...
    #          (LH_clay_norm + LH_SMR * HPT * (Dry and SMR or echem and (notDry))]
#--------------------------------------------------------------------------------------------
...
############################################################################
#CONVERSION CHECK: GO OVER THIS CODE AGAIN
    #    Converting life_mat to an array? What exactly are you indexing here?  #                                                                        #
    life_mat = (1 + r) ** np.arange(2, Lifetime + 2, 1) # (2:1:(Lifetime+1))  assume 1 year build time
############################################################################

    OpEx_tot = OpEx * 0
    OpEx_tot = OpEx_tot + OpEx * sum(1 / life_mat)

    OpExMat_tot = np.zeros(len(OpExMat), dtype=float) # CHECK CONVERSION: WHAT DATA TYPE DO YOU WANT THIS TO BE?
    
    OpExMat_tot = OpExMat_tot + OpExMat * sum(1 / life_mat)
    Cap_sum = sum((TPYcal)/ life_mat)
